...
ld <- "dat/ldsc/eur_w_ld_chr"
hm3 <- "dat/ldsc/eur_w_ld_chr/w_hm3.snplist"
...
genomewide_mr <- function(M_id, M_gwas, M_N, pheno_id, pheno_gwas, ref_bfile, out_dir) {
    ...
    exposure <- exposure[exposure$pval.exposure < 5e-6, ] # 5e-8
    exposure <- exposure[
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # MRlap
    # Sample overlap + weak instrument + winner's curse sensitivity
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    mrlap.res <- tryCatch(
        MRlap::MRlap(
            ...
            MR_pruning_LD = 0.001,
            MR_plink = Sys.which("plink"),
            MR_bfile = ref_bfile,
            save_logfiles = FALSE,
            verbose = TRUE
        ),
        error = function(e) {
            print(paste0("[CONCERN] MRlap failed for ", M_id, ": ", e$message))
            return(NULL)
        }
    )

    if (!is.null(mrlap.res)) {
        print(paste0(
            "[TRACKING] MRlap corrected beta: ",
            mrlap.res$MRcorrection$corrected_effect,
            " | p = ",
            mrlap.res$MRcorrection$corrected_effect_p
        ))
    }
    ...
    res.temp[, `:=`(
        MRlap_n_instruments = NA_integer_,
        MRlap_observed_beta = NA_real_,
        MRlap_observed_se = NA_real_,
        MRlap_observed_pval = NA_real_,
        MRlap_corrected_beta = NA_real_,
        MRlap_corrected_se = NA_real_,
        MRlap_corrected_pval = NA_real_,
        MRlap_p_difference = NA_real_,
        MRlap_cross_trait_intercept = NA_real_
    )]

    if (!is.null(mrlap.res)) {

        res.temp[, `:=`(
            MRlap_n_instruments = mrlap.res$MRcorrection$m_IVs,
            MRlap_observed_beta = mrlap.res$MRcorrection$observed_effect,
            MRlap_observed_se = mrlap.res$MRcorrection$observed_effect_se,
            MRlap_observed_pval = mrlap.res$MRcorrection$observed_effect_p,
            MRlap_corrected_beta = mrlap.res$MRcorrection$corrected_effect,
            MRlap_corrected_se = mrlap.res$MRcorrection$corrected_effect_se,
            MRlap_corrected_pval = mrlap.res$MRcorrection$corrected_effect_p,
            MRlap_p_difference = mrlap.res$MRcorrection$p_difference,
            MRlap_cross_trait_intercept = mrlap.res$LDSC$int_crosstrait
        )]
    }
    ...
    keep_cols <- c(
        ...
        "MRlap_observed_se",
        "MRlap_observed_pval",
        "MRlap_corrected_beta",
        "MRlap_corrected_se",
        "MRlap_corrected_pval",
        "MRlap_p_difference",
        "MRlap_cross_trait_intercept"
    )
